...
export function FormularioRegistroComponent() {
    ...
    const { register, handleSubmit, watch, control, formState: { errors }, setValue } = useForm<FormularioRegistro>({
        defaultValues: {
            nombre: "",
            fechaNacimiento: "",
            tipoPiel: "",
        ...
        <form onSubmit={handleSubmit(onSubmit)} className="space-y-5">
            ...
            <div className="grid gap-5 sm:grid-cols-2">
                <div className="space-y-2">
                    ...
                    {errors.fechaNacimiento && <span className="text-destructive text-sm">{errors.fechaNacimiento.message}</span>}
                </div>
                ...
                <div className="space-y-2">
                    ...
                    <Controller
                      name="tipoPiel"
                      control={control}
                      rules={{ required: campoObligatorio }}
                      render={({ field }) => (
                        <Select onValueChange={field.onChange} value={field.value}>
                          <SelectTrigger className="w-full">
                            <SelectValue placeholder={t("campos.tipoPiel.placeholder")} />
                          </SelectTrigger>
                          <SelectContent>
                            {tiposPiel.map((tipo) => (
                              <SelectItem key={tipo.value} value={tipo.value}>
                                {t(`campos.tipoPiel.opciones.${tipo.value}`)}
                              </SelectItem>
                            ))}
                          </SelectContent>
                        </Select>
                    ...
                    />
                    {errors.tipoPiel && <span className="text-destructive text-sm">{errors.tipoPiel.message}</span>}
                </div>
            </div>
            ...
            <div className="space-y-2">
                ...
                />
                {errors.probadoSkinCare && <span className="text-destructive text-sm">{errors.probadoSkinCare.message}</span>}
            </div>

            <div className="space-y-2">
                <p className="text-sm font-medium">{t("campos.comoEnteroDeNosotros.label")}</p>
                <Controller
                  name="comoEnteroDeNosotros"
                  control={control}
                  rules={{ required: campoObligatorio }}
                  render={({ field }) => (
                    <Select onValueChange={field.onChange} value={field.value}>
                      <SelectTrigger className="w-full">
                        <SelectValue placeholder={t("campos.comoEnteroDeNosotros.placeholder")} />
                      </SelectTrigger>
                      <SelectContent>
                        {formasEnteroDeNosotros.map((forma) => (
                          <SelectItem key={forma.value} value={forma.value}>
                            {t(`campos.comoEnteroDeNosotros.opciones.${forma.value}`)}
                          </SelectItem>
                        ))}
                      </SelectContent>
                    </Select>
                ...
                />
                {errors.comoEnteroDeNosotros && <span className="text-destructive text-sm">{errors.comoEnteroDeNosotros.message}</span>}
            </div>
            ...
            <div className="grid gap-5 sm:grid-cols-2">
                <div className="space-y-2">
                    ...
                    />
                    {errors.contrasenia && <span className="text-destructive text-sm">{errors.contrasenia.message}</span>}
                </div>
                ...
                <div className="space-y-2">
                    ...
                    {errors.confirmarContrasenia && (
                        <span className="text-destructive text-sm">{errors.confirmarContrasenia.message}</span>
                    )}
                </div>
            </div>
